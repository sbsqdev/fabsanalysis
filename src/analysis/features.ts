  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['Глаза хорошо раскрыты и симметричны — это выразительный взгляд. Специалист покажет, как подчеркнуть форму и удлинить взгляд'],
    confidence: dynConf(base, qualityFactor),
    limitations: [
...
export function analyzeNose(
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['Нос гармонично вписан в пропорции лица. Специалист оценит тонкие возможности для улучшения контура'],
    confidence: dynConf(base, qualityFactor),
    limitations,
  };
}
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : hasProfiles
          ? ['Скуловые зоны выглядят пропорционально. Специалист покажет варианты лёгкого акцентирования и лифтинга']
          : ['Скуловые зоны сбалансированы. Добавьте профильные снимки — это откроет больше возможностей для анализа'],
    confidence: dynConf(base, qualityFactor),
    limitations,
  };
}
...
export function analyzeJaw(
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['Линия челюсти выглядит сбалансированно. Специалист оценит возможности для более чёткого контура лица'],
    confidence: dynConf(base, qualityFactor),
    limitations,
  };
}
...
export function analyzeLips(
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['Губы хорошо сбалансированы — пропорции верхней и нижней части в норме. Специалист ProFace покажет, как подчеркнуть форму и добавить объём'],
    confidence: dynConf(base, qualityFactor),
    limitations: [
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : hasProfiles
          ? ['Подбородок пропорционально вписан в нижнюю треть лица. Тонкая коррекция поможет ещё чётче обозначить контур']
          : ['Подбородок выглядит гармонично. Добавьте профильный снимок — это позволит точнее оценить выступание и форму'],
    confidence: dynConf(base, qualityFactor),
    limitations,
  };
}
...
export function analyzeSkin(
  ...
  return {
    ...
    recommendations:
      recommendations.length > 0
        ? recommendations
        : ['Кожа выглядит достаточно равномерно. Для детального анализа тона и текстуры лучше подойдёт очная консультация косметолога'],
    confidence,
    limitations: [
  ...
  if (hasProfiles && profileAngles.length > 0) {
    ...
    return {
      ...
      recommendations:
        recommendations.length > 0
          ? recommendations
          : ['Шейно-подбородочный контур выглядит чётко. Специалист оценит дополнительные возможности для улучшения профиля'],
      confidence,
      limitations: [

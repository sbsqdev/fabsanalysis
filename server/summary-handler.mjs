// Production handler: POST /api/summary
// Plain-language face summary via DeepSeek (OpenAI-compatible). Non-streaming.
// Mirrors api/summary.ts.
...
function resolveDeepSeekModel() {
  return process.env.DEEPSEEK_MODEL?.trim() || readEnvValueFromFiles('DEEPSEEK_MODEL') || 'deepseek-chat';
}
...
function buildUserPayload(body, language) {
  const lines = [];
...
export async function handleSummary(req, res) {
  ...
  if (!apiKey) {
    ...
    res.end(JSON.stringify({ error: 'DEEPSEEK_API_KEY не настроен на сервере.' }));
    return;
  }

...
function buildDeviationPrompt(preset, deviations) {
  const parts = [SURGICAL_CONTEXT[preset]];
...
async function uploadDataUrl(dataUrl, cacheKey) {
  pruneUploadCache();
  if (cacheKey) {
...
function buildPrompt(preset, deviations) {
...
export async function handleTransform(req, res) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Ошибка трансформации';
    console.error('[/api/transform] Error:', message, '| status:', error?.status, '| body:', JSON.stringify(error?.body || {}));
    res.writeHead(500, { 'Content-Type': 'application/json' });
